...
export interface MapInfoFile {
    ...
    version: number,
    data: MapInfo[],
    ancestors: string[] // Used for clearing old scoreboard objectives when combining datapacks
}
...
export function isValidMapInfoFile(value: unknown): value is MapInfoFile {
    ...
    ('ancestors' in value && typeof value.ancestors === 'object' && value.ancestors instanceof Array) &&
    (value.ancestors.every(x => (typeof x === 'string') && x));
}
...
export async function generateDataPack(mapData: MapInfo[], version: number, ancestors: string[] = []) {
    ...
    if (ancestors.length) {
        ...
        funcFolder.file('clean_old.mcfunction', ancestors.map(x => generateCleanFunction(x)).join('') + `scoreboard players set $cleaned i_${id} 1\n`);
    }
